...
def sklearn_multiclass_prediction(mode, X_train, y_train, X_test):
    ...
    '''
    if mode == 'ovr':
        clf = multiclass.OneVsRestClassifier(estimator=svm.LinearSVC(random_state=12345))
    elif mode == 'ovo':
        clf = multiclass.OneVsOneClassifier(estimator=svm.LinearSVC(random_state=12345))
    elif mode == 'crammer':
